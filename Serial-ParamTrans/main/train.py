...
def validate_official(args, data_loader, model, global_stats, mode='dev'):
    ...
    examples = 0
    sources, sources2, hypotheses, references, copy_dict = dict(), dict(), dict(), dict(), dict()
    with torch.no_grad():
        pbar = tqdm(data_loader)
        for idx, ex in enumerate(pbar):
            ...
            src2_sequences = [code for code in ex['code2_text']]
            examples += batch_size
            for key, src, src2, pred, tgt in zip(ex_ids, src_sequences, src2_sequences, predictions, targets):
                hypotheses[key] = [pred]
...
def eval_accuracies(hypotheses, references, copy_info, sources=None, sources2=None,
    ...
    for key in references.keys():
        ...
        if fw:
            ...
            if sources is not None:
                logobj['SBT'] = sources[key]
            if sources2 is not None:
                logobj['method_summary'] = sources2[key]
